        ...
        var TokenExecutor = class {
          ...
          eatTokenAndForwardLookahead(tokenType) {
            ...
            if (token.type !== tokenType) {
              throw new SyntaxError(`kya kar hai tu??...Unexpected token: "${token.value}", expected : "${tokenType}"`);
            }
    ...
    var BinaryExpression = class {
      visitNode(node) {
        ...
        if (node.type == import_bhai_lang_parser.NodeType.BinaryExpression) {
          ...
          left2 = InterpreterModule.getVisitor(node.left.type).visitNode(node.left);
          right2 = InterpreterModule.getVisitor(node.right.type).visitNode(node.right);
